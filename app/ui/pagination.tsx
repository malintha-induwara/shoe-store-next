...
function PaginationNumber({ page, href, isActive, position }: { page: number | string; href: string; position?: "first" | "last" | "middle" | "single"; isActive: boolean }) {
  let className = "flex h-10 w-10 items-center justify-center text-sm border border-gray-400"; ;
...
function PaginationArrow({ href, direction, isDisabled }: { href: string; direction: "left" | "right"; isDisabled?: boolean }) {
  let className = "flex h-10 w-10 items-center justify-center rounded-md border ";

  if (isDisabled) {
    className += " pointer-events-none text-gray-300";
  } else {
    className += " hover:bg-gray-100 border-gray-400";
  }
