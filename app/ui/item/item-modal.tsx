...
export function ItemModal({
  ...
  const renderImage = () => {
    ...
    if (initialData?.image && !imageRemoved) {
      return (
        <div className="relative">
          <Image
            src={`${process.env.NEXT_PUBLIC_BLOB_URL}${initialData.image}`}
            alt="Product"
            width={128}
            height={128}
  ...
  return (
    <dialog
      ref={dialogRef}
      className="bg-white rounded-lg shadow-xl w-full max-w-2xl fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 backdrop:bg-gray-800/70"
    >
